...
public class ClusterPointRouter implements MessageListener, ServerStreamChannelMessageListener {
    ...
    public ClusterPointRouter(ClusterPointRepository<TargetClusterPoint> targetClusterPointRepository,
        ...
        Preconditions.checkNotNull(null != targetClusterPointRepository, new NullPointerException("targetClusterPointRepository may not be null"));
        Preconditions.checkNotNull(null != defaultRouteHandler, new NullPointerException("defaultRouteHandler may not be null"));
        Preconditions.checkNotNull(null != streamRouteHandler, new NullPointerException("streamRouteHandler may not be null"));
    ...
    public StreamCode handleStreamCreate(ServerStreamChannelContext streamChannelContext, StreamCreatePacket packet) {
        ...
        if (request == null) {
            ...
            return handleStreamRouteCreate((TCommandTransfer)request, streamChannelContext);
        } else {
    ...
    private boolean handleRouteRequest(TCommandTransfer request, RequestPacket requestPacket, PinpointSocket pinpointSocket) {
        ...
        TCommandTransferResponse response = routeHandler.onRoute(new RequestEvent(request, pinpointSocket.getRemoteAddress(), requestPacket.getRequestId(), command));
    ...
    private StreamCode handleStreamRouteCreate(TCommandTransfer request, ServerStreamChannelContext streamChannelContext) {
        byte[] payload = request.getPayload();
        TBase<?,?> command = deserialize(payload);
        if (command == null) {
            return StreamCode.TYPE_UNKNOWN;
        ...
        TCommandTransferResponse response = streamRouteHandler.onRoute(new StreamEvent(request, streamChannelContext, command));
