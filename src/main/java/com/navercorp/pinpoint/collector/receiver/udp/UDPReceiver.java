...
public class UDPReceiver implements DataReceiver {
    ...
    private void handleRejectedExecutionException(RejectedExecutionException ree) {
        ...
        final int mod = 100;
        if ((error % mod) == 0) {
            logger.warn("RejectedExecutionCount={}", error, ree);
        }
    }
    ...
    private DatagramSocket createSocket(int receiveBufferSize) {
        try {
            ...
            if (logger.isWarnEnabled()) {
                final int checkReceiveBufferSize = datagramSocket.getReceiveBufferSize();
    ...
    public void start() {
        ...
        bindSocket(datagramSocket, bindAddress, port);
