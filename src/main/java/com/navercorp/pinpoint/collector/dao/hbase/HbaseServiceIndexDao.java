...
@Repository
public class HbaseServiceIndexDao implements ServiceIndexDao {
    ...
    public void update(String appName, long timestamp, TopoLine topoLine) {
        ...
        List<XNode> nodeList = hbaseTemplate.get(HBaseTables.SERVICEINDEX, rowKey, HBaseTables.SERVICEINDEX_CF_N, xNodeMapper);
        List<XLink> linkList = hbaseTemplate.get(HBaseTables.SERVICEINDEX, rowKey, HBaseTables.SERVICEINDEX_CF_L, xLinkMapper);

        TopoLine tmpTopoline = new TopoLine(nodeList, linkList);
        tmpTopoline.setTimestamp(timeslot);
        ...
        if (!(nodeList.isEmpty() || linkList.isEmpty())) {
            ...
            logger.info("add new record for topoline, appName={}, topo={}, timeslot={}",appName, topoLine.toString(), timeslot);
        }

        Put put = new Put(rowKey);
        for (XNode xNode : tmpTopoline.getXNodes()) {
            put.add(HBaseTables.SERVICEINDEX_CF_N, BytesUtils.toBytes(xNode.getName()), xNode.writeValue());
        }

        for (XLink xLink : tmpTopoline.getXLinks()) {
            byte[] qualifier = createLinkQualifier(xLink.getFrom(), xLink.getTo());
            put.add(HBaseTables.SERVICEINDEX_CF_L, qualifier, xLink.writeValue());
        }

        //insert new data
        synchronized (syncObject) {
            hbaseTemplate.put(HBaseTables.SERVICEINDEX, put);
        }
    }
